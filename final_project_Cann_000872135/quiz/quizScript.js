    ...
    for (i = 0; i < questions.length; i++) {
        table +=
            `   <h4>Question ${questions[i].getElementsByTagName("qnumber")[0].childNodes[0].nodeValue}</h4>
                <p>${questions[i].getElementsByTagName("qtitle")[0].childNodes[0].nodeValue}</p>   
                <input type="radio" id="q${i+1}_1" name="q${i+1}" value="a">
                <label for="q${i+1}_1">A) ${questions[i].getElementsByTagName("a")[0].childNodes[0].nodeValue}</label> 
                <br>
                <input type="radio" id="q${i+1}_2" name="q${i+1}" value="b">
                <label for="q${i+1}_2">B) ${questions[i].getElementsByTagName("b")[0].childNodes[0].nodeValue}</label>
                <br>
                <input type="radio" id="q${i+1}_3" name="q${i+1}" value="c">
                <label for="q${i+1}_3">C) ${questions[i].getElementsByTagName("c")[0].childNodes[0].nodeValue}</label>
                <br>
...
function checkResults() {
    ...
    for (i = 0; i < quizLength; i++) {
        userAnswer[i] === answers[i] ?
            console.log('correct' + correct++) : console.log(`incorrect`);
    }
    document.getElementById('displayResults').innerHTML = `<h3>You got ${correct}/${quizLength} Correct!\n${(correct/quizLength)*100}%</h3> <button type="button" class="btn btn-warning" onclick="location.reload() ">Reset Test</button>`;
...
function giveFeedback(userAnswer, quizLength) {
    ...
    for (i = 0; i < quizLength; i++) {
        if (userAnswer[i] === answers[i]) {
            ...
            result.push(false)
            let wrong = document.querySelector(`label[for="q${i+1}_${returnNumber(userAnswer[i])}"]`);
    ...
    let iterator = 0;
    result.forEach(() => {
        let right = document.querySelector('label[for="q' + qNum + '_' +
            returnNumber(answers[iterator]) + '"]');
